...
# Julien: this was done already, see my comment in the exercise 1
...
## Julien: better to save object from day1-2 and reload it?
...
# 
# spe <- spe_subset
## Julien: is it a good idea to subset spots randomly? Is it really necessary here given that spe is already a subsetted object?
...
## Julien: nothing removed here on my side...
...
## Top 10 HVGs
top_HVG <- row.names(dec[order(dec$bio, decreasing = T), ])[1:10]
...
> Julien: wouldn't it be actually very interesting? And then compare SVGs and HVGs. The issue is that it can be long to calculate... What if we reduce to a subset of HVGs and to run nnSVG / Moran's I?

```{r}
#BiocManager::install("nnSVG")
#library(nnSVG)

## Subset to top 100 HVGs (faster)
spe_subset <- spe[rowData(spe)$Symbol %in% getTopHVGs(dec, n = 100), ]
## TO DO: subset spots too?

# set random seed for number generation in order to make results reproducible
spe_subset <- nnSVG(spe_subset,
                    n_neighbors = 5, ## default 10 better, but slower
                    BPPARAM = BiocParallel::MulticoreParam(4),
                    verbose=TRUE) 
## Problem: This is quite long!
## TO DO: speed up by reducing even more number of spots? Or give students a pre-computed list of SVGs to download?

## Results
head(rowData(spe_subset), 3)
table(rowData(spe_subset)$padj <= 0.05) ## all of them!

hist(rowData(spe_subset)$pval,
  breaks = 50, xlab = "P-value"
)
## Maybe more interesting to look at top-ranking ones?
head(rowData(spe_subset)[order(rowData(spe_subset)$rank), ])

top_nnSVG <- rowData(spe_subset)$Symbol[rowData(spe_subset)$rank <= 10]

gs <- rowData(spe)$Symbol[match(gs, rownames(spe))]
...
################################################################################ 
# Moran's I
# renv::install("Voyager")
library(Voyager)

sfe <- toSpatialFeatureExperiment(spe_subset)
knn8 <- findSpatialNeighbors(sfe, type="centroids", method="knearneigh", k=8)
colGraph(sfe, "knn8") <- knn8
...
# We can visualize the 10 genes with highest Moran’s
rowData(spe_subset)$moran_sample01 <- rowData(sfe)$moran_sample01
top_moran <- rowData(spe_subset)$Symbol[order(rowData(spe_subset)$moran_sample01, decreasing = TRUE)][1:10]
all(top_nnSVG %in% top_moran)
...
ps <- lapply(top_moran, \(.) {
    plotCoords(spe, 
        ...
        feature_names = "Symbol", 
        assay_name = "logcounts") })
unit <- ggplot2::unit ## masked when loading Voyager
patchwork::wrap_plots(ps, nrow = 2) & 
  theme(legend.key.width = unit(0.4, "lines"), 
        legend.key.height = unit(0.8, "lines")) & 
  scale_color_gradientn(colors = rev(hcl.colors(9, "Rocket")))

## Both measures are showing very similar thing...
df <- rowData(spe_subset) |> as.data.frame()
ggplot(data = df, aes(x=moran_sample01, y=LR_stat)) +
...
t-SNE is a non-linear dimensionality reduction technique widely used for visualizing high-dimensional datasets. It tries placing similar data points close together in a low-dimensional map, while dissimilar points are placed far apart. This has many limitations, and distances ont he tSNE should be interpreted with caution, but it can be helpful at illsutrating patterns in the data.
> Julien: is this making sense here? Given how controversial tSNE and UMAPs are, maybe we could simply avoid them here. I find much more helpful to plot the PCs on the tissue, see above
