...
```{r}
p1 <- plotVisium(spe, annotate = "sum", point_size = 0.5)
```

```{r}
p1 <- plotCoords(spe, annotate = "sum", point_size = 0.5)
p2 <- plotCoords(spe, annotate = "detected", point_size = 0.5)
p1 | p2
```

```{r}
spe$loglibsize <- log1p(spe$sum)
p1 <- plotCoords(spe, annotate = "loglibsize", point_size = 0.5)
p2 <- plotCoords(spe, annotate = "sum", point_size = 0.5)
