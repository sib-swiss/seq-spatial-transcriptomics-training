...
In this exercise, we extend our analysis to multi-sample, multi-condition spatial transcriptomics data. With multiple samples per condition (e.g., 2 CRC and 2 normal tissues), we can ask not only: 
...
Genes showing such changes are called **differential spatial pattern (DSP)** genes. Using the `DESpace` package, we will learn how to identify such genes, that may be spatially variable in one condition but not in another, or it could be spatially variable in both conditions, but with different spatial patterns.
...
`DESpace::dsp_test()` first aggregate spot- or cell-level counts into pseudobulk counts for each (sample, domain) combination, then test whether the interaction term (condition X domain) in the model are different from zero.
...
- Take the top 3 DSP genes and visualize their spatial expression across all samples `ggspavis::plotCoords()`.
- How would you interpret these patterns?
:::
...
**UBE2C** is a marker of cell proliferation and tends to show broad, strong expression in tumor epithelium but limited to crypt bases in normal tissue. **SPIB** is associated with B cells and dendritic cells, often appears in immune cell aggregates that become more prominent in cancer tissue. **TGFBI**, cancer-associated fibroblasts and ECM remodeling, and typically appears stronger and more localized in tumor stroma than in healthy colon. 
:::
