...
In order to use the big spatial statistics library `sf` we convert the `SpatialExperiment` object into a `SpatialFeatureExperiment` object [@Moses2025.02.24.640007]. In addition to the `SpatialExperiment` object it contains simple feature categories such as `Geometries` and `Graphs`. In this 10x Visium Dataset the `Geometries` stored are the points of the spots
...
Visium HD can provide subcellular resulution. We can segment cells from this regular lattice of pixels and approximate the cells by their centroid. This in turn can be described be an _irregular lattice_ of the cell types in space or even be analysed as a point process of the cell centroid locations. 
...
- The $y$-coordinate is flipped to before. This is due to the way that visium data is acquired. The visium slide is placed on top of the tissue but the H&E image is acquired from below the tissue.
...
Then, we will plot the local Moran's $I$ values in space. 
...
The grey dashed lines indicate the mean expreession of the "PIGR" expression in the spots themselves and among their neighbours. According to to these means the spots are categorised categorised. 

We can plot the categories mapped back in space. We notice that there s quite a large fraction of regions that constitute as "high-high" for all the genes in the same region.
...
We notice that there is one region which shows high bivariate Moran's I. This is also the region where both univeriate Moran's I scatterplot values were "high-high". The regions which were individually for each gene high do not show up here.
