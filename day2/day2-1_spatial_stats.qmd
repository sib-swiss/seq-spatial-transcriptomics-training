...
A very famous measure of global spatial autocorrelation is called Moran's $I$. It assess correlation of a gene with itself given a defined weight matrix [@moran1950notes]. 
...
Then, we extract the 20 most highly variable genes

```{r}
stats <- scran::modelGeneVar(sfe, subset.row = 1:2000)
hvg <- scran::getTopHVGs(stats, prop = 0.02)
```

Next, we calculate global Moran's I for these 20 genes
